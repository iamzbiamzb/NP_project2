...
public:
	...
	void boardcastusermsg(vector<boardcastmsg> allusermsg, int fd ) {
	...
	void who( int id , int fd ) {
		vector<user> tmpuservec(user_vec.begin(),user_vec.end());
		...
		for ( int i = 0 ; i < tmpuservec.size() ; i++ ) {
			string ss ;
			ss += to_string(tmpuservec[i].get_id()) ;
			ss += "	";
			ss += tmpuservec[i].get_name() ;
			ss += "	";
			ss += tmpuservec[i].get_ipport();
			ss += "	";

			//ss += 
			if ( tmpuservec[i].get_id() == id ) {
				ss += "<-me\n";
			}else ss += "\n";
